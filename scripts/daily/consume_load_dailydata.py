...
def load_dailyweather_data(df):
    try:
        ...
        for _, row in current_df.iterrows():
            ...
            # FIXED: Use the complete datetime for mapping, not just the date
            date_id = None
            for db_datetime, db_date_id in date_mapping.items():
                if db_datetime.strftime('%Y-%m-%d %H:%M') == row['datetime'].strftime('%Y-%m-%d %H:%M'):
                    date_id = db_date_id
                    break
